...
define('DB_USER', 'root');              // Tvoj DB užívateľ
define('DB_PASS', '');                  // Tvoje DB heslo
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($conn->connect_error) {
            throw new Exception("Chyba pripojenia: " . $conn->connect_error);
        }
        ...
        if (empty($name)) {
            ...
            if (!$stmt) {
                throw new Exception("Chyba pri príprave príkazu: " . $conn->error);
            }
            ...
            if ($stmt->execute()) {
                ...
                // VOLITEĽNÉ: Odoslanie potvrdenia emailom
                ...
                $response['message'] = 'Vyskytla sa chyba pri ukladaní. Skúste neskôr.';
            }
